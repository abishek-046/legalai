...
import { FileText, Trash2, Eye, Calendar, CheckCircle, XCircle, AlertTriangle } from 'lucide-react'
import { format } from 'date-fns'

const STATUS_CFG = {
  Legal:          { bg: 'rgba(74,222,128,0.1)',  border: 'rgba(74,222,128,0.25)', text: '#4ade80' },
  Illegal:        { bg: 'rgba(239,68,68,0.1)',   border: 'rgba(239,68,68,0.25)',  text: '#f87171' },
  'Needs Review': { bg: 'rgba(251,191,36,0.1)',  border: 'rgba(251,191,36,0.25)', text: '#fbbf24' },
}

const RISK_CFG = {
  Low:    { text: '#4ade80', label: 'Low Risk' },
  Medium: { text: '#fbbf24', label: 'Medium Risk' },
  High:   { text: '#f87171', label: 'High Risk' },
}
...
export default function ReportCard({ report, onDelete }) {
  ...
  const sc = STATUS_CFG[status] || STATUS_CFG['Needs Review']
  const rc = RISK_CFG[report.riskLevel] || RISK_CFG.Medium

  return (
    <div className="rounded-2xl p-5 hover:border-gold-400/20 transition-all duration-300 group animate-fade-in"
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h3 className="font-semibold text-white truncate text-sm" title={report.filename}>
              {report.filename}
            </h3>
      ...
      <div className="mt-4 pt-3 border-t border-white/5 space-y-2">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-xs font-medium" style={{ color: rc.text }}>
            {rc.label}
          </span>
          {report.confidenceScore != null && (
            <span className="text-xs text-slate-600 ml-auto">{report.confidenceScore}% confidence</span>
          )}
        </div>
